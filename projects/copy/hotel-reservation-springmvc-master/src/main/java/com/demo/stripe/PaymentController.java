...
public class PaymentController {
    ...
    Response createCharge(String email, String token, Double amount) {
        ...
        //create charge
        String convertedAmount = String.valueOf(Math.round(amount));
        String chargeId = stripeService.createCharge(email, token, Integer.parseInt(convertedAmount)); //$9.99 USD
        
        if (chargeId == null) {
            return new Response(false, "An error occurred while trying to create a charge.");
        }

        this.emailService.sendMail(email, "HOTEL RESERVATION", chargeId);
