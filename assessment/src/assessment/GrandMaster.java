	...
	//	Date date = new Date();
		...
		for(int a = 0; a < 5; a++ )
			...
			SimpleDateFormat simpleDateFormat =
			        new SimpleDateFormat(pattern, new Locale("Eng", "SA"));
...
System.out.println("Ass: " + Result.formatAsCustomString(date, Integer.parseInt(result)));
		...
		*/
		System.out.println("Ass: " + Result.formatAsCustomString(new Date(), 5));
	}
...
class Result {
    public static String formatAsCustomString(Date date, int number) {
    	...
    	String [] newDObjects = d.split("");
    	
    	String result ="";
    	
    	for(int a = 0; a < newDObjects.length; a ++) {
    		
    		result += newDObjects[a].trim();
    	}
    	
     
        String num = String.valueOf(number);
        return leftPad(num, 5,'0') + "-" + result;
    }
